...
public class DebugRendererPathfinding implements DebugRenderer.IDebugRenderer {
      ...
      if (!this.pathMap.isEmpty()) {
         ...
         for(Integer integer : this.pathMap.keySet()) {
            ...
            if (!(this.addDistanceToPlayer(pathpoint) > 40.0F)) {
               WorldRenderer.func_189696_b((new AxisAlignedBB((double)((float)pathpoint.x + 0.25F), (double)((float)pathpoint.y + 0.25F), (double)pathpoint.z + 0.25D, (double)((float)pathpoint.x + 0.75F), (double)((float)pathpoint.y + 0.75F), (double)((float)pathpoint.z + 0.75F))).offset(-this.xo, -this.yo, -this.zo), 0.0F, 1.0F, 0.0F, 0.5F);
               ...
               for(int j = 0; j < path.getCurrentPathLength(); ++j) {
                  ...
                  if (!(this.addDistanceToPlayer(pathpoint1) > 40.0F)) {
                     ...
                     WorldRenderer.func_189696_b((new AxisAlignedBB((double)((float)pathpoint1.x + 0.5F - f), (double)((float)pathpoint1.y + 0.01F * (float)j), (double)((float)pathpoint1.z + 0.5F - f), (double)((float)pathpoint1.x + 0.5F + f), (double)((float)pathpoint1.y + 0.25F + 0.01F * (float)j), (double)((float)pathpoint1.z + 0.5F + f))).offset(-this.xo, -this.yo, -this.zo), f1, 0.0F, f2, 0.5F);
                  }
               }
            }
         }
         ...
         for(Integer integer1 : this.pathMap.keySet()) {
            ...
            for(PathPoint pathpoint3 : path1.getClosedSet()) {
               if (!(this.addDistanceToPlayer(pathpoint3) > 40.0F)) {
                  DebugRenderer.renderDebugText(String.format("%s", pathpoint3.nodeType), (double)pathpoint3.x + 0.5D, (double)pathpoint3.y + 0.75D, (double)pathpoint3.z + 0.5D, p_190060_1_, -65536);
                  DebugRenderer.renderDebugText(String.format(Locale.ROOT, "%.2f", pathpoint3.costMalus), (double)pathpoint3.x + 0.5D, (double)pathpoint3.y + 0.25D, (double)pathpoint3.z + 0.5D, p_190060_1_, -65536);
               }
            }

            for(PathPoint pathpoint4 : path1.getOpenSet()) {
               if (!(this.addDistanceToPlayer(pathpoint4) > 40.0F)) {
                  DebugRenderer.renderDebugText(String.format("%s", pathpoint4.nodeType), (double)pathpoint4.x + 0.5D, (double)pathpoint4.y + 0.75D, (double)pathpoint4.z + 0.5D, p_190060_1_, -16776961);
                  DebugRenderer.renderDebugText(String.format(Locale.ROOT, "%.2f", pathpoint4.costMalus), (double)pathpoint4.x + 0.5D, (double)pathpoint4.y + 0.25D, (double)pathpoint4.z + 0.5D, p_190060_1_, -16776961);
               }
            }
            ...
            for(int k = 0; k < path1.getCurrentPathLength(); ++k) {
               ...
               if (!(this.addDistanceToPlayer(pathpoint2) > 40.0F)) {
                  DebugRenderer.renderDebugText(String.format("%s", pathpoint2.nodeType), (double)pathpoint2.x + 0.5D, (double)pathpoint2.y + 0.75D, (double)pathpoint2.z + 0.5D, p_190060_1_, -1);
                  DebugRenderer.renderDebugText(String.format(Locale.ROOT, "%.2f", pathpoint2.costMalus), (double)pathpoint2.x + 0.5D, (double)pathpoint2.y + 0.25D, (double)pathpoint2.z + 0.5D, p_190060_1_, -1);
               }
            }
         }
   ...
   private float addDistanceToPlayer(PathPoint p_190066_1_) {
      return (float)(Math.abs((double)p_190066_1_.x - this.player.posX) + Math.abs((double)p_190066_1_.y - this.player.posY) + Math.abs((double)p_190066_1_.z - this.player.posZ));
   }
}
