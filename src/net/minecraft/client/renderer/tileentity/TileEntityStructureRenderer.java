...
public class TileEntityStructureRenderer extends TileEntityRenderer<TileEntityStructure> {
   public void render(TileEntityStructure p_199341_1_, double p_199341_2_, double p_199341_4_, double p_199341_6_, float p_199341_8_, int p_199341_9_) {
      if (Minecraft.getInstance().player.canUseCommandBlock() || Minecraft.getInstance().player.isSpectator()) {
         ...
         BlockPos blockpos = p_199341_1_.getPosition();
         BlockPos blockpos1 = p_199341_1_.getStructureSize();
         if (blockpos1.getX() >= 1 && blockpos1.getY() >= 1 && blockpos1.getZ() >= 1) {
            if (p_199341_1_.getMode() == StructureMode.SAVE || p_199341_1_.getMode() == StructureMode.LOAD) {
               double d0 = 0.01D;
               double d1 = (double)blockpos.getX();
               double d2 = (double)blockpos.getZ();
               double d6 = p_199341_4_ + (double)blockpos.getY() - 0.01D;
               double d9 = d6 + (double)blockpos1.getY() + 0.02D;
               double d3;
               double d4;
               switch(p_199341_1_.getMirror()) {
               case LEFT_RIGHT:
               ...
               double d8;
               double d10;
               switch(p_199341_1_.getRotation()) {
               case CLOCKWISE_90:
                  d5 = p_199341_2_ + (d4 < 0.0D ? d1 - 0.01D : d1 + 1.0D + 0.01D);
                  d7 = p_199341_6_ + (d3 < 0.0D ? d2 + 1.0D + 0.01D : d2 - 0.01D);
                  d8 = d5 - d4;
                  d10 = d7 + d3;
                  break;
               case CLOCKWISE_180:
                  d5 = p_199341_2_ + (d3 < 0.0D ? d1 - 0.01D : d1 + 1.0D + 0.01D);
                  d7 = p_199341_6_ + (d4 < 0.0D ? d2 - 0.01D : d2 + 1.0D + 0.01D);
                  d8 = d5 - d3;
                  d10 = d7 - d4;
                  break;
               case COUNTERCLOCKWISE_90:
                  d5 = p_199341_2_ + (d4 < 0.0D ? d1 + 1.0D + 0.01D : d1 - 0.01D);
                  d7 = p_199341_6_ + (d3 < 0.0D ? d2 - 0.01D : d2 + 1.0D + 0.01D);
                  d8 = d5 + d4;
                  d10 = d7 - d3;
                  break;
               default:
                  d5 = p_199341_2_ + (d3 < 0.0D ? d1 + 1.0D + 0.01D : d1 - 0.01D);
                  d7 = p_199341_6_ + (d4 < 0.0D ? d2 + 1.0D + 0.01D : d2 - 0.01D);
                  d8 = d5 + d3;
                  d10 = d7 + d4;
               }
               ...
               if (p_199341_1_.getMode() == StructureMode.SAVE || p_199341_1_.showsBoundingBox()) {
                  this.renderBox(tessellator, bufferbuilder, d5, d6, d7, d8, d9, d10, 255, 223, 127);
               }

               if (p_199341_1_.getMode() == StructureMode.SAVE && p_199341_1_.showsAir()) {
                  this.renderInvisibleBlocks(p_199341_1_, p_199341_2_, p_199341_4_, p_199341_6_, blockpos, tessellator, bufferbuilder, true);
                  this.renderInvisibleBlocks(p_199341_1_, p_199341_2_, p_199341_4_, p_199341_6_, blockpos, tessellator, bufferbuilder, false);
               }
   ...
   private void renderInvisibleBlocks(TileEntityStructure p_190054_1_, double p_190054_2_, double p_190054_4_, double p_190054_6_, BlockPos p_190054_8_, Tessellator p_190054_9_, BufferBuilder p_190054_10_, boolean p_190054_11_) {
      ...
      for(BlockPos blockpos2 : BlockPos.getAllInBox(blockpos1, blockpos1.add(p_190054_1_.getStructureSize()).add(-1, -1, -1))) {
         ...
         if (flag || flag1) {
            ...
            if (p_190054_11_) {
               WorldRenderer.func_189698_a(p_190054_10_, d0, d1, d2, d3, d4, d5, 0.0F, 0.0F, 0.0F, 1.0F);
            } else if (flag) {
               WorldRenderer.func_189698_a(p_190054_10_, d0, d1, d2, d3, d4, d5, 0.5F, 0.5F, 1.0F, 1.0F);
            } else {
               WorldRenderer.func_189698_a(p_190054_10_, d0, d1, d2, d3, d4, d5, 1.0F, 0.25F, 0.25F, 1.0F);
            }
         }
      }
