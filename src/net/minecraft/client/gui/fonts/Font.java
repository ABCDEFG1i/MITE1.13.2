...
public class Font implements AutoCloseable {
   ...
   public void setGlyphProviders(List<IGlyphProvider> p_211570_1_) {
      ...
      p_211570_1_.stream().filter(set::contains).forEach(this.glyphProviders::add);
   }
   ...
   private TexturedGlyph createTexturedGlyph(IGlyphInfo p_211185_1_) {
      for(FontTexture fonttexture : this.textures) {
         TexturedGlyph texturedglyph = fonttexture.createTexturedGlyph(p_211185_1_);
         if (texturedglyph != null) {
            return texturedglyph;
         }
      }

      FontTexture fonttexture1 = new FontTexture(new ResourceLocation(this.id.getNamespace(), this.id.getPath() + "/" + this.textures.size()), p_211185_1_.isColored());
      this.textures.add(fonttexture1);
      this.textureManager.loadTexture(fonttexture1.getTextureLocation(), fonttexture1);
      TexturedGlyph texturedglyph1 = fonttexture1.createTexturedGlyph(p_211185_1_);
   ...
   public TexturedGlyph func_211188_a(IGlyph p_211188_1_) {
      ...
      return charlist != null && !charlist.isEmpty() ? this.func_211187_a(charlist.get(field_212462_d.nextInt(charlist.size()))) : this.fallbackGlyph;
   }
}
