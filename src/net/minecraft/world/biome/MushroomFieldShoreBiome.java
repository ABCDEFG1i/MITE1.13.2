...
public final class MushroomFieldShoreBiome extends Biome {
   public MushroomFieldShoreBiome() {
      super((new Biome.BiomeBuilder()).surfaceBuilder(new CompositeSurfaceBuilder(DEFAULT_SURFACE_BUILDER, MYCELIUM_DIRT_GRAVEL_SURFACE)).precipitation(Biome.RainType.RAIN).category(Biome.Category.MUSHROOM).depth(0.0F).scale(0.025F).temperature(0.9F).downfall(1.0F).waterColor(4159204).waterFogColor(329011).parent(
              null));
      ...
      this.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, createCompositeFeature(Feature.TWO_FEATURE_CHOICE, new TwoFeatureChoiceConfig(Feature.BIG_RED_MUSHROOM, IFeatureConfig.NO_FEATURE_CONFIG, Feature.BIG_BROWN_MUSHROOM, IFeatureConfig.NO_FEATURE_CONFIG), AT_SURFACE, new FrequencyConfig(1)));
      ...
      this.addSpawn(EnumCreatureType.CREATURE, new Biome.SpawnListEntry(EntityType.MOOSHROOM, 8, 4, 8));
