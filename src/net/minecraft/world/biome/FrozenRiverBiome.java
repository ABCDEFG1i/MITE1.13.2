...
public final class FrozenRiverBiome extends Biome {
   public FrozenRiverBiome() {
      super((new Biome.BiomeBuilder()).surfaceBuilder(new CompositeSurfaceBuilder(DEFAULT_SURFACE_BUILDER, GRASS_DIRT_GRAVEL_SURFACE)).precipitation(Biome.RainType.SNOW).category(Biome.Category.RIVER).depth(-0.5F).scale(0.0F).temperature(0.0F).downfall(0.5F).waterColor(3750089).waterFogColor(329011).parent(
              null));
      ...
      this.addSpawn(EnumCreatureType.MONSTER, new Biome.SpawnListEntry(EntityType.DROWNED, 1, 1, 1));
