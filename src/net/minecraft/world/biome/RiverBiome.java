...
public final class RiverBiome extends Biome {
   public RiverBiome() {
      super((new Biome.BiomeBuilder()).surfaceBuilder(new CompositeSurfaceBuilder(DEFAULT_SURFACE_BUILDER, GRASS_DIRT_GRAVEL_SURFACE)).precipitation(Biome.RainType.RAIN).category(Biome.Category.RIVER).depth(-0.5F).scale(0.0F).temperature(0.5F).downfall(0.5F).waterColor(4159204).waterFogColor(329011).parent(
              null));
      ...
      this.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, createCompositeFeature(Feature.SEA_GRASS, new SeaGrassConfig(48, 0.4D), TOP_SOLID_ONCE, IPlacementConfig.NO_PLACEMENT_CONFIG));
      ...
      this.addSpawn(EnumCreatureType.MONSTER, new Biome.SpawnListEntry(EntityType.DROWNED, 100, 1, 1));
