...
public class OverworldDimension extends Dimension {
   ...
   public boolean canDropChunk(int p_186056_1_, int p_186056_2_) {
      return !this.world.isSpawnChunk(p_186056_1_, p_186056_2_) && super.canDropChunk(p_186056_1_, p_186056_2_);
   }
   ...
   public IChunkGenerator<? extends IChunkGenSettings> createChunkGenerator() {
      ...
      if (worldtype == WorldType.FLAT) {
         FlatGenSettings flatgensettings = FlatGenSettings.func_210835_a(new Dynamic<>(NBTDynamicOps.INSTANCE, this.world.getWorldInfo().getGeneratorOptions()));
         ...
         return chunkgeneratortype1.create(this.world, biomeprovidertype.create(singlebiomeprovidersettings), chunkgeneratortype1.createChunkGenSettings());
      } else if (worldtype != WorldType.BUFFET) {
         OverworldGenSettings overworldgensettings = chunkgeneratortype4.createChunkGenSettings();
         OverworldBiomeProviderSettings overworldbiomeprovidersettings = biomeprovidertype1.createSettings().setWorldInfo(this.world.getWorldInfo()).setSettings(overworldgensettings);
         ...
         if (jsonobject.has("biome_source") && jsonobject.getAsJsonObject("biome_source").has("type") && jsonobject.getAsJsonObject("biome_source").has("options")) {
            ...
            if (BiomeProviderType.CHECKERBOARD.getKey().equals(resourcelocation)) {
               ...
               CheckerboardBiomeProviderSettings checkerboardbiomeprovidersettings = biomeprovidertype2.createSettings().func_206860_a(abiome).func_206861_a(j);
            ...
            if (BiomeProviderType.VANILLA_LAYERED.getKey().equals(resourcelocation)) {
               OverworldBiomeProviderSettings overworldbiomeprovidersettings1 = biomeprovidertype1.createSettings().setSettings(new OverworldGenSettings()).setWorldInfo(this.world.getWorldInfo());
         ...
         if (jsonobject.has("chunk_generator") && jsonobject.getAsJsonObject("chunk_generator").has("options")) {
            if (jsonobject.getAsJsonObject("chunk_generator").getAsJsonObject("options").has("default_block")) {
               ...
               Block block = IRegistry.field_212618_g.func_82594_a(new ResourceLocation(s));
               if (block != null) {
            ...
            if (jsonobject.getAsJsonObject("chunk_generator").getAsJsonObject("options").has("default_fluid")) {
               ...
               Block block1 = IRegistry.field_212618_g.func_82594_a(new ResourceLocation(s1));
               if (block1 != null) {
   ...
   public BlockPos findSpawn(int p_206921_1_, int p_206921_2_, boolean p_206921_3_) {
      ...
      if (p_206921_3_ && !iblockstate.getBlock().isIn(BlockTags.VALID_SPAWN)) {
         return null;
      } else {
