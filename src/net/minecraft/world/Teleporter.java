...
public class Teleporter {
    ...
    private void makePortal(Entity p_85188_1_, Block portalBlock) {
        double d0 = -1.0D;
                                ...
                                for (int k4 = 0; k4 < 4; ++k4) {
                                    for (int l4 = -1; l4 < 4; ++l4) {
                                        ...
                                        if (l4 < 0 && !this.world.getBlockState(
                                                blockpos$mutableblockpos).getMaterial().isSolid() || l4 >= 0 && !this.world.isAirBlock(
                            ...
                            for (int l7 = i2; l7 < i2 + 2; ++l7) {
                                ...
                                for (int i10 = 0; i10 < 4; ++i10) {
                                    for (int k10 = -1; k10 < 4; ++k10) {
                                        ...
                                        if (k10 < 0 && !this.world.getBlockState(
                                                blockpos$mutableblockpos).getMaterial().isSolid() || k10 >= 0 && !this.world.isAirBlock(
                                                blockpos$mutableblockpos)) {
                                            continue label214;
                                        }
                                    }
        ...
        IBlockState block_portal = portalBlock.getDefaultState().with(BlockNetherPortal.AXIS,
                l6 == 0 ? EnumFacing.Axis.Z : EnumFacing.Axis.X);

        for (int k8 = 0; k8 < 2; ++k8) {
            for (int j9 = 0; j9 < 3; ++j9) {
                blockpos$mutableblockpos.setPos(i6 + k8 * l6, k2 + j9, k6 + k8 * i3);
                this.world.setBlockState(blockpos$mutableblockpos, block_portal, 18);
            }
        }
    ...
    public boolean placeInExistingPortal(Entity p_180620_1_, float p_180620_2_) {
            ...
            for (int i1 = -128; i1 <= 128; ++i1) {
                ...
                for (int j1 = -128; j1 <= 128; ++j1) {
                    for (BlockPos blockpos1 = blockpos3.add(i1, this.world.getActualHeight() - 1 - blockpos3.getY(), j1); blockpos1.getY() >= 0; blockpos1 = blockpos2) {
                        ...
                        if (BlockTags.PORTALS.contains(this.world.getBlockState(blockpos1).getBlock())) {
                            for (blockpos2 = blockpos1.down();
                                 BlockTags.PORTALS.contains(this.world.getBlockState(blockpos2).getBlock());
        ...
        if (d0 >= 0.0D) {
            ...
            BlockPattern.PatternHelper blockpattern$patternhelper = ((BlockNetherPortal) Blocks.NETHER_PORTAL).createPatternHelper(
                    this.world, blockpos);
            ...
            if (p_180620_1_ instanceof EntityPlayerMP) {
                ((EntityPlayerMP) p_180620_1_).connection.setPlayerLocation(d5, d6, d7, p_180620_1_.rotationYaw,
                        p_180620_1_.rotationPitch);
    ...
    public void placeInPortal(Entity p_180266_1_, float p_180266_2_, DimensionType from) {
        DimensionType to = this.world.dimension.getType();
        if (this.world.dimension.getType() != DimensionType.THE_END) {
            if (!this.placeInExistingPortal(p_180266_1_, p_180266_2_)) {
                if (to == DimensionType.OVERWORLD) {
                    this.makePortal(p_180266_1_, Blocks.UNDERWORLD_PORTAL);
                }
                if (to == DimensionType.UNDERWORLD) {
                    if (from == DimensionType.OVERWORLD) {
                        this.makePortal(p_180266_1_, Blocks.OVERWORLD_PORTAL);
                    } else {
                        this.makePortal(p_180266_1_, Blocks.NETHER_PORTAL);
                    }
                }
    ...
    void removeStalePortalLocations(long p_85189_1_) {
        if (p_85189_1_ % 100L == 0L) {
            long i = p_85189_1_ - 300L;

            this.destinationCoordinateCache.values().removeIf(
                    teleporter$portalposition -> teleporter$portalposition == null || teleporter$portalposition.lastUpdateTime < i);
        }
    ...
    public class PortalPosition extends BlockPos {
        ...
        PortalPosition(BlockPos p_i45747_2_, long p_i45747_3_) {
