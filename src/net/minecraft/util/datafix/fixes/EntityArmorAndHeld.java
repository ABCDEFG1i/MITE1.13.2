...
public class EntityArmorAndHeld extends DataFix {
   ...
   public TypeRewriteRule makeRule() {
      return this.func_206323_b(this.getInputSchema().getTypeRaw(TypeReferences.ITEM_STACK));
   }

   private <IS> TypeRewriteRule func_206323_b(Type<IS> p_206323_1_) {
      ...
      return this.fixTypeEverywhereTyped("EntityEquipmentToArmorAndHandFix", this.getInputSchema().getType(TypeReferences.ENTITY), this.getOutputSchema().getType(TypeReferences.ENTITY), (p_207448_4_) -> {
         ...
         if (optional1.isPresent()) {
            ...
            if (!dynamic.get("ArmorDropChances").isPresent()) {
               Dynamic<?> dynamic2 = dynamic.emptyMap().merge(dynamic.createFloat(iterator.next().getNumberValue(0).floatValue())).merge(dynamic.createFloat(iterator.next().getNumberValue(0).floatValue())).merge(dynamic.createFloat(iterator.next().getNumberValue(0).floatValue())).merge(dynamic.createFloat(iterator.next().getNumberValue(0).floatValue()));
