...
public class PotionBrewing {
   ...
   protected static boolean isItemConversionReagent(ItemStack p_185203_0_) {
      ...
      for(int j = POTION_ITEM_CONVERSIONS.size(); i < j; ++i) {
         if ((POTION_ITEM_CONVERSIONS.get(i)).reagent.test(p_185203_0_)) {
            return true;
         }
      }
   ...
   protected static boolean isTypeConversionReagent(ItemStack p_185211_0_) {
      ...
      for(int j = POTION_TYPE_CONVERSIONS.size(); i < j; ++i) {
         if ((POTION_TYPE_CONVERSIONS.get(i)).reagent.test(p_185211_0_)) {
            return true;
         }
      }
   ...
   protected static boolean hasItemConversions(ItemStack p_185206_0_, ItemStack p_185206_1_) {
      ...
      for(int j = POTION_ITEM_CONVERSIONS.size(); i < j; ++i) {
         ...
         if (mixpredicate.input == item && mixpredicate.reagent.test(p_185206_1_)) {
            return true;
         }
      }
   ...
   protected static boolean hasTypeConversions(ItemStack p_185209_0_, ItemStack p_185209_1_) {
      ...
      for(int j = POTION_TYPE_CONVERSIONS.size(); i < j; ++i) {
         ...
         if (mixpredicate.input == potiontype && mixpredicate.reagent.test(p_185209_1_)) {
            return true;
         }
      }
   ...
   public static ItemStack doReaction(ItemStack p_185212_0_, ItemStack p_185212_1_) {
      if (!p_185212_1_.isEmpty()) {
         PotionType potiontype = PotionUtils.getPotionFromItem(p_185212_1_);
         Item item = p_185212_1_.getItem();
         int i = 0;

         for(int j = POTION_ITEM_CONVERSIONS.size(); i < j; ++i) {
            PotionBrewing.MixPredicate<Item> mixpredicate = POTION_ITEM_CONVERSIONS.get(i);
            if (mixpredicate.input == item && mixpredicate.reagent.test(p_185212_0_)) {
               return PotionUtils.addPotionToItemStack(new ItemStack((IItemProvider)mixpredicate.output), potiontype);
            }
         }
         ...
         for(int k = POTION_TYPE_CONVERSIONS.size(); i < k; ++i) {
            ...
            if (mixpredicate1.input == potiontype && mixpredicate1.reagent.test(p_185212_0_)) {
   ...
   private static void func_196207_a(Item p_196207_0_, Item p_196207_1_, Item p_196207_2_) {
      if (!(p_196207_0_ instanceof ItemPotion)) {
         throw new IllegalArgumentException("Expected a potion, got: " + IRegistry.field_212630_s.func_177774_c(p_196207_0_));
      } else if (!(p_196207_2_ instanceof ItemPotion)) {
         throw new IllegalArgumentException("Expected a potion, got: " + IRegistry.field_212630_s.func_177774_c(p_196207_2_));
      } else {
   ...
   private static void func_196208_a(Item p_196208_0_) {
      if (!(p_196208_0_ instanceof ItemPotion)) {
         throw new IllegalArgumentException("Expected a potion, got: " + IRegistry.field_212630_s.func_177774_c(p_196208_0_));
      } else {
