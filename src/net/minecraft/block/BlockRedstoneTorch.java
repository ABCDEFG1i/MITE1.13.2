...
public class BlockRedstoneTorch extends BlockTorch {
   ...
   private static final Map<IBlockReader, List<BlockRedstoneTorch.Toggle>> BURNED_TORCHES = new java.util.WeakHashMap<>(); // FORGE - fix vanilla MC-101233

   BlockRedstoneTorch(Block.Properties builder) {
      super(builder);
      this.setDefaultState(this.stateContainer.getBaseState().with(LIT, Boolean.TRUE));
   }
    ...
    * this method is unrelated to {randomTick} and {@link #needsRandomTick}, and will always be called regardless
   ...
   private static boolean isBurnedOut(World p_176598_0_, BlockPos worldIn, boolean pos) {
      List<BlockRedstoneTorch.Toggle> list = BURNED_TORCHES.computeIfAbsent(p_176598_0_, k -> Lists.newArrayList());

      if (pos) {
         list.add(new BlockRedstoneTorch.Toggle(worldIn.toImmutable(), p_176598_0_.getTotalWorldTime()));
      }
