...
public class BlockSkullWither extends BlockSkull {
   ...
   public void onBlockPlacedBy(World p_180633_1_, BlockPos p_180633_2_, IBlockState p_180633_3_, @Nullable EntityLivingBase p_180633_4_, ItemStack p_180633_5_) {
      ...
      if (tileentity instanceof TileEntitySkull) {
         checkWitherSpawn(p_180633_1_, p_180633_2_, (TileEntitySkull)tileentity);
      }
   ...
   public static void checkWitherSpawn(World p_196298_0_, BlockPos p_196298_1_, TileEntitySkull p_196298_2_) {
      ...
      if (flag && p_196298_1_.getY() >= 2 && p_196298_0_.getDifficulty() != EnumDifficulty.PEACEFUL && !p_196298_0_.isRemote) {
         ...
         if (blockpattern$patternhelper != null) {
            ...
            for(EntityPlayerMP entityplayermp : p_196298_0_.getEntitiesWithinAABB(EntityPlayerMP.class, entitywither.getEntityBoundingBox().grow(50.0D))) {
   ...
   public static boolean func_196299_b(World p_196299_0_, BlockPos p_196299_1_, ItemStack p_196299_2_) {
      if (p_196299_2_.getItem() == Items.WITHER_SKELETON_SKULL && p_196299_1_.getY() >= 2 && p_196299_0_.getDifficulty() != EnumDifficulty.PEACEFUL && !p_196299_0_.isRemote) {
         return func_196297_e().match(p_196299_0_, p_196299_1_) != null;
      } else {
         return false;
      }
   }
   ...
   protected static BlockPattern func_196296_d() {
      if (field_196300_c == null) {
         field_196300_c = FactoryBlockPattern.start().aisle("^^^", "###", "~#~").where('#', BlockWorldState.hasState(BlockStateMatcher.forBlock(Blocks.SOUL_SAND))).where('^', BlockWorldState.hasState(BlockStateMatcher.forBlock(Blocks.WITHER_SKELETON_SKULL).or(BlockStateMatcher.forBlock(Blocks.WITHER_SKELETON_WALL_SKULL)))).where('~', BlockWorldState.hasState(BlockMaterialMatcher.forMaterial(Material.AIR))).build();
      }
   ...
   protected static BlockPattern func_196297_e() {
      if (field_196301_y == null) {
         field_196301_y = FactoryBlockPattern.start().aisle("   ", "###", "~#~").where('#', BlockWorldState.hasState(BlockStateMatcher.forBlock(Blocks.SOUL_SAND))).where('~', BlockWorldState.hasState(BlockMaterialMatcher.forMaterial(Material.AIR))).build();
      }
