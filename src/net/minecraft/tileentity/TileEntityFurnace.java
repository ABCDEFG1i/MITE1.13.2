...
public class TileEntityFurnace extends TileEntityLockable implements ISidedInventory, IRecipeHolder,
        IRecipeHelperPopulator, ITickable {
    private static final int[] SLOTS_BOTTOM = new int[]{2, 1};
    private static final int[] SLOTS_SIDES = new int[]{1};
    private static final int[] SLOTS_TOP = new int[]{0};
    private static final Map<Item, Integer> heatLevelMap = Maps.newLinkedHashMap();
    private static final Map<Item, Integer> map = Maps.newLinkedHashMap();

    static {
        setHeatLevel(heatLevelMap, Items.LAVA_BUCKET, 3);
        setHeatLevel(heatLevelMap, Blocks.COAL_BLOCK, 2);
        setHeatLevel(heatLevelMap, Items.BLAZE_ROD, 4);
        setHeatLevel(heatLevelMap, Items.COAL, 2);
        setHeatLevel(heatLevelMap, Items.CHARCOAL, 0);
        setHeatLevel(heatLevelMap, ItemTags.LOGS, 1);
        setHeatLevel(heatLevelMap, ItemTags.PLANKS, 1);
        setHeatLevel(heatLevelMap, ItemTags.WOODEN_STAIRS, 1);
        setHeatLevel(heatLevelMap, ItemTags.WOODEN_SLABS, 1);
        setHeatLevel(heatLevelMap, ItemTags.WOODEN_TRAPDOORS, 1);
        setHeatLevel(heatLevelMap, ItemTags.WOODEN_PRESSURE_PLATES, 1);
        setHeatLevel(heatLevelMap, Blocks.OAK_FENCE, 1);
        setHeatLevel(heatLevelMap, Blocks.BIRCH_FENCE, 1);
        setHeatLevel(heatLevelMap, Blocks.SPRUCE_FENCE, 1);
        setHeatLevel(heatLevelMap, Blocks.JUNGLE_FENCE, 1);
        setHeatLevel(heatLevelMap, Blocks.DARK_OAK_FENCE, 1);
        setHeatLevel(heatLevelMap, Blocks.ACACIA_FENCE, 1);
        setHeatLevel(heatLevelMap, Blocks.OAK_FENCE_GATE, 1);
        setHeatLevel(heatLevelMap, Blocks.BIRCH_FENCE_GATE, 1);
        setHeatLevel(heatLevelMap, Blocks.SPRUCE_FENCE_GATE, 1);
        setHeatLevel(heatLevelMap, Blocks.JUNGLE_FENCE_GATE, 1);
        setHeatLevel(heatLevelMap, Blocks.DARK_OAK_FENCE_GATE, 1);
        setHeatLevel(heatLevelMap, Blocks.ACACIA_FENCE_GATE, 1);
        setHeatLevel(heatLevelMap, Blocks.NOTE_BLOCK, 1);
        setHeatLevel(heatLevelMap, Blocks.BOOKSHELF, 1);
        setHeatLevel(heatLevelMap, Blocks.JUKEBOX, 1);
        setHeatLevel(heatLevelMap, Blocks.CHEST, 1);
        setHeatLevel(heatLevelMap, Blocks.TRAPPED_CHEST, 1);
        setHeatLevel(heatLevelMap, ItemTags.CRAFTING_TABLE, 1);
        setHeatLevel(heatLevelMap, Blocks.DAYLIGHT_DETECTOR, 1);
        setHeatLevel(heatLevelMap, ItemTags.BANNERS, 1);
        setHeatLevel(heatLevelMap, Items.BOW, 0);
        setHeatLevel(heatLevelMap, Items.FISHING_ROD, 0);
        setHeatLevel(heatLevelMap, Blocks.LADDER, 1);
        setHeatLevel(heatLevelMap, Items.SIGN, 0);
        setHeatLevel(heatLevelMap, Items.WOODEN_SHOVEL, 1);
        setHeatLevel(heatLevelMap, Items.WOODEN_SWORD, 1);
        setHeatLevel(heatLevelMap, ItemTags.WOODEN_DOORS, 1);
        setHeatLevel(heatLevelMap, ItemTags.BOATS, 0);
        setHeatLevel(heatLevelMap, ItemTags.WOOL, 1);
        setHeatLevel(heatLevelMap, ItemTags.WOODEN_BUTTONS, 1);
        setHeatLevel(heatLevelMap, Items.STICK, 0);
        setHeatLevel(heatLevelMap, ItemTags.SAPLINGS, 0);
        setHeatLevel(heatLevelMap, Items.BOWL, 0);
        setHeatLevel(heatLevelMap, ItemTags.CARPETS, 1);
        setHeatLevel(heatLevelMap, Blocks.DRIED_KELP_BLOCK, 1);
    ...
    private static int getHeatLevel(ItemStack itemStack) {
        if (itemStack.isEmpty()) {
            return 0;
        } else {
            Item item = itemStack.getItem();
            return getHeatLevels().getOrDefault(item, 0);
        }
    }

    private static Map<Item, Integer> getHeatLevels() {
        return heatLevelMap;
    }
    ...
    private static void setHeatLevel(Map<Item, Integer> map, IItemProvider target, int heatLevel) {
        map.put(target.asItem(), heatLevel);
    }

    private static void setHeatLevel(Map<Item, Integer> p_201563_0_, Tag<Item> p_201563_1_, int p_201563_2_) {
        for (Item item : p_201563_1_.getAllElements()) {
            p_201563_0_.put(item, p_201563_2_);
        }
    ...
    private boolean canSmelt(@Nullable IRecipe p_201566_1_) {
        if (!this.furnaceItemStacks.get(0).isEmpty() && p_201566_1_ != null) {
            ...
            if (p_201566_1_ instanceof ITieredRecipe) {
                if (!isBurning()) {
                    if (getHeatLevel(this.furnaceItemStacks.get(1)) > this.maxHeatLevel) {
                        return false;
                    }
                }
                if (((ITieredRecipe) p_201566_1_).getTier() > this.heatLevel) {
                    return false;
                }
            }
            if (itemstack.isEmpty()) {
    ...
    public Container createContainer(InventoryPlayer p_174876_1_, EntityPlayer p_174876_2_) {
        return new ContainerFurnace(p_174876_1_, this, this.heatLevel);
    }
    ...
    public ITextComponent getName() {
        return this.furnaceCustomName != null ? this.furnaceCustomName : new TextComponentTranslation(
                "container.furnace");
    }
    ...
    public void setInventorySlotContents(int p_70299_1_, ItemStack p_70299_2_) {
        ...
        boolean flag = !p_70299_2_.isEmpty() && p_70299_2_.isItemEqual(itemstack) && ItemStack.areItemStackTagsEqual(
                p_70299_2_, itemstack);
        ...
        if (p_70299_1_ == 1) {
            if (!this.isBurning()) {
                this.heatLevel = getHeatLevel(p_70299_2_);
                this.markDirty();
            }
        }
    ...
    public boolean isUsableByPlayer(EntityPlayer p_70300_1_) {
        if (this.world.getTileEntity(this.pos) != this) {
            ...
            return !(p_70300_1_.getDistanceSq((double) this.pos.getX() + 0.5D, (double) this.pos.getY() + 0.5D,
    ...
    public boolean isItemValidForSlot(int p_94041_1_, ItemStack p_94041_2_) {
        if (p_94041_1_ == 2) {
            ...
            return isItemFuel(p_94041_2_) || SlotFurnaceFuel.isBucket(
                    p_94041_2_) && itemstack.getItem() != Items.BUCKET;
        }
    }
    ...
    public void readFromNBT(NBTTagCompound p_145839_1_) {
        ...
        this.heatLevel = p_145839_1_.getShort("HeatLevel");
    ...
    public NBTTagCompound writeToNBT(NBTTagCompound p_189515_1_) {
        ...
        p_189515_1_.setShort("HeatLevel", (short) this.heatLevel);
    ...
    public TileEntityFurnace setMaxHeatLevel(int maxHeatLevel) {
    ...
    private void smeltItem(@Nullable IRecipe p_201565_1_) {
        if (p_201565_1_ != null && this.canSmelt(p_201565_1_)) {
            ...
            if (itemstack.getItem() == Blocks.WET_SPONGE.asItem() && !this.furnaceItemStacks.get(1)
                    .isEmpty() && this.furnaceItemStacks.get(1).getItem() == Items.BUCKET) {
        ...
        if (!this.world.isRemote) {
            ...
            if (flag != this.isBurning()) {
                ...
                this.world.setBlockState(this.pos,
                        this.world.getBlockState(this.pos).with(BlockFurnace.LIT, Boolean.valueOf(this.isBurning())),
                        3);
            }
            if (!isBurning()) {
                int heatLevel = getHeatLevel(this.furnaceItemStacks.get(1));
